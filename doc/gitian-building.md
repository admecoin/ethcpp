...
git clone https://github.com/cryptofugu/bcschain-core --recursive
...
```bash
sudo cp /home/debian/bcschain-core/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bcschain-core/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/cryptofugu/bcschain-core
...
For example:
```bash
URL=https://github.com/cryptofugu/bcschain-core.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bcschain-core=${COMMIT} --url bcschain-core=${URL} ../bcschain-core/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bcschain-core=${COMMIT} --url bcschain-core=${URL} ../bcschain-core/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bcschain-core=${COMMIT} --url bcschain-core=${URL} ../bcschain-core/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-bcs:
./bin/gbuild --commit bcschain-core=${COMMIT},cpp-eth-bcs=${ETHCOMMIT} --url bcschain-core=${URL},cpp-eth-bcs=${ETHURL} ../bcschain-core/contrib/gitian-descriptors/gitian-linux.yml
```
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bcschain-core/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/cryptofugu/bcs-detached-sigs.git

BTCPATH=/some/root/path/bcschain-core
SIGPATH=/some/root/path/bcs-detached-sigs

./bin/gbuild --url bcschain-core=${BTCPATH},signature=${SIGPATH} ../bcschain-core/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
[cryptofugu/gitian.sigs](https://github.com/cryptofugu/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to our email (hello@cryptofugu.com) and we will commit them.
