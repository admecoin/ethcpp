...
#define paternVersion "bcs-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

BCSVersionChecker::BCSVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

BCSVersionChecker::~BCSVersionChecker()
{

}

bool BCSVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> BCSVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(BCS_RELEASES)));
    QEventLoop event;
...
Version BCSVersionChecker::getMaxReleaseVersion()
{
