...
void BCSDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool BCSDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule BCSDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t BCSDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t BCSDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t BCSDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t BCSDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool BCSDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void BCSDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void BCSDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void BCSDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void BCSDGP::createParamsInstance(){
...
dev::Address BCSDGP::getAddressForBlock(unsigned int blockHeight){
...
void BCSDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void BCSDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void BCSDGP::parseStorageOneUint64(uint64_t& value){
...
void BCSDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule BCSDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
