...
inline void initState(){
    ...
    const std::string dirBCS = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<BCSState>(new BCSState(dev::u256(0), BCSState::openDB(dirBCS, hashDB, dev::WithExisting::Trust), dirBCS + "/bcsDB", dev::eth::BaseState::Empty));
...
inline dev::Address createBCSAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline BCSTransaction createBCSTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    BCSTransaction txEth;
    if(recipient == dev::Address()){
        txEth = BCSTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = BCSTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<BCSTransaction> txs){
