...
void genesisLoading(){
    ...
    dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::bcsMainNetwork, 999)));
...
BOOST_AUTO_TEST_CASE(checking_returndata_opcode_after_fork){
    ...
    txs.push_back(createBCSTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    txs.push_back(createBCSTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    executeBC(txs);

    // Call upgrade to
    dev::Address proxy = createBCSAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<BCSTransaction> txsCall;
    txsCall.push_back(createBCSTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    executeBC(txsCall);

    // Call mint
    std::vector<BCSTransaction> txsMint;
    txsMint.push_back(createBCSTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    auto result = executeBC(txsMint);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);

    // Call balance of
    std::vector<BCSTransaction> txsbalance;
    txsbalance.push_back(createBCSTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
BOOST_AUTO_TEST_CASE(checking_constantinople_after_fork){
    ...
    // Create contract
    std::vector<BCSTransaction> txs;
    txs.push_back(createBCSTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs);

    // Call is it constantinople
    dev::Address proxy = createBCSAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<BCSTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createBCSTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
