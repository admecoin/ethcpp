...
void checkResult(bool isCreation, std::vector<BCSTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    BCSTxConverter converter(transaction, NULL);
    ExtractBCSTX bcsTx;
    BOOST_CHECK(converter.extractionBCSTransactions(bcsTx));
    std::vector<BCSTransaction> result = bcsTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(bcstxconverter_tests, TestingSetup)
