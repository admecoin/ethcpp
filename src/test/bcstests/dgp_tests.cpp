...
void contractLoading(){
    ...
    dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::bcsMainNetwork, 1400)));
...
void createTestContractsAndBlocks(TestChain100Setup* testChain100Setup, const valtype& code1, const valtype& code2, const valtype& code3, dev::Address addr){
    ...
    txs.push_back(createBCSTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, addr, 0));
    txs.push_back(createBCSTransaction(code1, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createBCSTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    auto result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createBCSTransaction(code2, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createBCSTransaction(code[4], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createBCSTransaction(code3, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createBCSTransaction(code[6], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test1){
    ...
    dev::eth::EVMSchedule schedule = bcsDGP.getGasSchedule(100);
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test2){
    ...
    contractLoading();
    BCSDGP bcsDGP(globalState.get());
    dev::eth::EVMSchedule schedule = bcsDGP.getGasSchedule(0);
    BOOST_CHECK(compareEVMSchedule(schedule, dev::eth::EIP158Schedule));
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test3){
    ...
    contractLoading();
    BCSDGP bcsDGP(globalState.get());
    dev::eth::EVMSchedule schedule = bcsDGP.getGasSchedule(1400);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<BCSTransaction> txs;
    txs.push_back(createBCSTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasScheduleDGP, 0));
    txs.push_back(createBCSTransaction(code[1], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createBCSTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasScheduleDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test2){
    ...
    BCSDGP bcsDGP(globalState.get());
    dev::eth::EVMSchedule schedule = bcsDGP.getGasSchedule(502); // After initializing the tests, the height of the chain 502
...
BOOST_AUTO_TEST_CASE(gas_schedule_passage_from_0_to_130_three_paramsInstance_test){
    ...
    for(size_t i = 0; i < 1300; i++){
        dev::eth::EVMSchedule schedule = bcsDGP.getGasSchedule(i);
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test1){
    ...
    contractLoading();
    BCSDGP bcsDGP(globalState.get());
    uint32_t blockSize = bcsDGP.getBlockSize(100);
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test2){
    ...
    contractLoading();
    BCSDGP bcsDGP(globalState.get());
    uint32_t blockSize = bcsDGP.getBlockSize(0);
    BOOST_CHECK(blockSize == DEFAULT_BLOCK_SIZE_DGP);
}
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<BCSTransaction> txs;
    txs.push_back(createBCSTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, BlockSizeDGP, 0));
    txs.push_back(createBCSTransaction(code[7], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createBCSTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, BlockSizeDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test2){
    ...
    BCSDGP bcsDGP(globalState.get());
    uint32_t blockSize = bcsDGP.getBlockSize(502);
...
BOOST_AUTO_TEST_CASE(block_size_passage_from_0_to_130_three_paramsInstance_test){
    ...
    BCSDGP bcsDGP(globalState.get());
    for(size_t i = 0; i < 1300; i++){
        uint32_t blockSize = bcsDGP.getBlockSize(i);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test1){
    ...
    contractLoading();
    BCSDGP bcsDGP(globalState.get());
    uint64_t minGasPrice = bcsDGP.getMinGasPrice(100);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test2){
    ...
    contractLoading();
    BCSDGP bcsDGP(globalState.get());
    uint64_t minGasPrice = bcsDGP.getMinGasPrice(0);
    BOOST_CHECK(minGasPrice == DEFAULT_MIN_GAS_PRICE_DGP);
}
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<BCSTransaction> txs;
    txs.push_back(createBCSTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasPriceDGP, 0));
    txs.push_back(createBCSTransaction(code[10], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createBCSTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasPriceDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test2){
    ...
    BCSDGP bcsDGP(globalState.get());
    uint64_t minGasPrice = bcsDGP.getMinGasPrice(502);
...
BOOST_AUTO_TEST_CASE(min_gas_price_passage_from_0_to_130_three_paramsInstance_test){
    ...
    BCSDGP bcsDGP(globalState.get());
    for(size_t i = 0; i < 1300; i++){
        uint64_t minGasPrice = bcsDGP.getMinGasPrice(i);
        std::function<bool(const uint64_t&, const uint64_t&)> func = compareUint64;
