...
class BCSNoExecCallDisabledTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        tx.vout = [CTxOut(int(COIN - 10000000), scriptPubKey=CScript([b"\x00", CScriptNum(100000), CScriptNum(BCS_MIN_GAS_PRICE), b"\x00", hex_str_to_bytes(contract_address), OP_CALL]))]
        tx = rpc_sign_transaction(self.node, tx)
        assert_raises_rpc_error(-26, "bad-tx-version-rootvm", self.node.sendrawtransaction, bytes_to_hex_str(tx.serialize()))
...
if __name__ == '__main__':
    BCSNoExecCallDisabledTest().main()
